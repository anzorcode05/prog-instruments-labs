...
def prepare_data(dataset_path, sent_col_name, label_col_name):
    """ Чтение предложений и меток из tsv файла """
...
class Language:
    """ Создание словаря на основе списка предложений и преобразование слов в числовое представление """
    def __init__(self):
        self.word2id = {}
        self.id2word = {}
...
def collate_fn(batch_data):
    """ Кастомная функция для организации данных внутри батча """
...
def get_wordvec(word2id, vec_file_path, vec_dim=50):
    """ Загрузка предобученных word vectors из txt файла """
    print("Начинаю загрузку векторов слов")
    ...
    with open(vec_file_path, "r", encoding="utf-8") as f:
        ...
        for line in lines:
            ...
            if found == len(word2id) - 1:  # Разрешаем не найти <unk>
                break
    print("Всего %d слов, из них %d найдены соответствующие векторы" % (len(word2id), found))
...
def make_dataloader(dataset_path="../dataset/kaggle-movie-review", sent_col_name="Phrase", label_col_name="Sentiment", batch_size=32, vec_file_path="./.vector_cache/glove.6B.50d.txt", debug=False):
    ...
    # Всего 18229 слов, из них 12769 найдены соответствующие векторы word_vectors = get_wordvec(X_language.word2id,
